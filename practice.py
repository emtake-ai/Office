...
FURNITURE_OBJECTS = {
    "chair", "bench", "couch", "bed", "dining table", "table", "desk"
}
...
WORK_OBJECTS = {
    "laptop", "keyboard", "mouse", "monitor", "tv", "desk", "chair"
}
...
def build_ontology_facts(object_counts):
    ...
    for obj, count in sorted(object_counts.items()):
        ...
        if obj in FURNITURE_OBJECTS:
            detected_furniture.append(obj)

        if obj in UTILITY_OBJECTS:
            detected_utility.append(obj)

    if detected_furniture:
        contexts.append("office_furniture_detected")
        for obj in detected_furniture:
            relations.append(f"{obj} is office furniture")

    if detected_utility:
        contexts.append("office_utility_detected")
        for obj in detected_utility:
...
def direct_answer(question, object_counts, ontology_text):
    ...
    furniture = [
        ...
        if obj in FURNITURE_OBJECTS and object_counts[obj] > 0
    ]

    utility = [
        obj for obj in object_counts
        if obj in UTILITY_OBJECTS and object_counts[obj] > 0
    ]
    ...
    if "environment" in q or "situation" in q or "current" in q:
        ...
        if person_count > 0:
            parts.append(f"{person_count} person(s) are detected")

        if furniture:
            parts.append("furniture: " + object_list_text(furniture, object_counts))

        if utility:
            parts.append("utility devices: " + object_list_text(utility, object_counts))

        if "office_working_detected" in ontology_text:
            parts.append("office work is detected")

        if parts:
            return "Office environment: " + "; ".join(parts) + "."

        return "No clear office environment is detected."
...
- If ontology says office_furniture_detected, describe detected furniture.
- If ontology says office_utility_detected, describe detected utilities.
- If ontology says office_environment_detected, describe the office using detected objects.
