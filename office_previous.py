...
def direct_answer(question, object_counts, face_names, ontology_text):
    ...
    if "who" in q or "name" in q:
        if face_names:
            return "Detected person name(s): " + ", ".join(face_names) + "."
        return "No known face is detected."
    ...
    if "environment" in q or "situation" in q or "current" in q:
        ...
        if face_names:
            parts.append("names: " + ", ".join(face_names))
    ...
    if "working" in q or "work" in q:
        if "office_working_detected" in ontology_text:
            if face_names:
                return ", ".join(face_names) + " is working."
...
def monitor_loop(cap, yolo, face_app, known_faces):
    global latest_object_counts, latest_face_names, latest_frame, latest_annotated_frame
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            face_names = recognize_faces(face_app, known_faces, frame)
        except Exception:
            time.sleep(MONITOR_INTERVAL)
            continue
